...
# ==============================================================================
# AGENTIC DEEP SEARCH IMPLEMENTATION NOTES
# ==============================================================================
#
# TODO: Implement agentic deep search for low-confidence queries
#
# OVERVIEW:
# When BM25 search returns low confidence (<0.40), trigger an agentic search
# where a small LLM reads continuum segments and finds information that's
# paraphrased, synthesized across messages, or otherwise missed by keyword search.
#
# ARCHITECTURE:
# ============
#
...
#     query: str,
#     entities: List[str],
#     bm25_results: List[Dict[str, Any]]  # Weak signals to guide search
# ) -> Dict[str, Any]:
#     """
#     Execute agentic deep search when BM25 has low confidence.
#
#     Flow:
#     1. Select continuum segments to load
#     2. Format segments for LLM reading
#     3. Call agent LLM to find information
#     4. Parse agent response and validate message IDs
#     5. Fetch cited messages and format results
#     """
#
# SEGMENT SELECTION STRATEGY (Hybrid Approach):
# =============================================
#
# Priority order for loading continuum chunks:
#
# 1. BM25 Hints (if any weak matches exist):
#    - Load ±3 days around low-confidence BM25 match timestamps
#    - Rationale: Weak signals often point to the right time period
#    - Example: If BM25 found "Mark" on Oct 7 with rank 0.15, load Oct 4-10
#
# 2. Temporal Parsing (if query has time references):
#    - Parse: "yesterday", "last week", "when we talked about X on Monday"
#    - Load those specific date ranges
#    - Rationale: User memory of timing is often accurate
#    - Libraries: dateutil.parser or simple regex for common phrases
#
# 3. Recent History Fallback:
#    - If no hints, load last 7 days of conversations
#    - Rationale: Most queries are about recent interactions
#
# 4. Limits:
#    - Max 5 continuum days to avoid context overflow
#    - ~80 messages total (~8K tokens at 100 tokens/message)
#    - Prioritize days with more message activity
#
# Implementation:
# ```python
# def _select_conversation_segments(
#     self,
#     query: str,
#     bm25_results: List[Dict[str, Any]]
# ) -> List[str]:  # Returns list of date strings "YYYY-MM-DD"
#
#     dates = []
#
#     # Strategy 1: BM25 hints
#     if bm25_results:
#         for result in bm25_results[:3]:  # Top 3 weak matches
#             date = result["created_at"].date()
#             # Add ±3 days
#             dates.extend([
#                 (date - timedelta(days=3)).isoformat(),
...
#             ])
#
#     # Strategy 2: Temporal parsing (TODO: implement)
#     # temporal_dates = self._parse_temporal_references(query)
#     # dates.extend(temporal_dates)
#
#     # Strategy 3: Recent fallback
#     if not dates:
...
# ====================
#
# Load messages by date (recommended approach):
# ```python
# def _load_conversation_segments(
#     self,
#     dates: List[str]
# ) -> Dict[str, List[Dict[str, Any]]]:
#     """Load messages grouped by date."""
#
#     return self._conversation_repo.get_messages_for_dates(
#         user_id=self.user_id,
#         dates=dates
#     )
#     # Returns: {"2025-10-07": [msg1, msg2, ...], ...}
# ```
#
# Alternative: Load by continuum_id if you want thread coherence
#
# FORMATTING FOR LLM:
# ==================
#
# Format continuum segments as readable text for the agent:
# ```python
# def _format_segments_for_agent(
#     self,
#     segments: Dict[str, List[Dict[str, Any]]]
# ) -> str:
#     """Format continuum segments for LLM reading."""
#
#     formatted = []
#     for date, messages in sorted(segments.items()):
#         formatted.append(f"\n=== Continuum from {date} ===\n")
#
#         for msg in messages:
#             role = msg["role"].upper()
...
# ================
#
# Use existing GenericProviderClient pattern:
# ```python
# def _get_agent_llm_client(self):
#     """Lazily instantiate agent LLM client."""
#     if not hasattr(self, '_agent_llm'):
#         from clients.llm_provider import GenericProviderClient
#         from clients.vault_client import get_api_key
#
#         self._agent_llm = GenericProviderClient(
...
# ```
#
# Cost: ~$0.001 per search (Gemini Flash Lite)
# Speed: 1-2 seconds
#
# AGENT SYSTEM PROMPT:
# ===================
#
# AGENT_SYSTEM_PROMPT = """You are a continuum search assistant.
#
# You will be given:
# 1. A search query from the user
# 2. Entities/proper nouns they're looking for
# 3. Continuum segments from their history
#
# Your task:
# - Read the continuum segments carefully
# - Find information matching the query
# - Look for EXACT matches, paraphrases, and synthesized information
# - Return message IDs where relevant information was found
#
# IMPORTANT:
# - Only cite message IDs that actually appear in the segments
# - Do NOT hallucinate or make up information
# - If information is spread across multiple messages, cite all relevant ones
# - Explain briefly what you found and where
#
...
#     "findings": [
#         {
#             "message_id": "full-uuid-from-segments",
#             "excerpt": "relevant text excerpt",
#             "match_type": "exact|paraphrase|synthesized",
#             "explanation": "Brief explanation of what this message contains"
#         }
#     ],
#     "reasoning": "Overall explanation of what you found"
# }
# """
#
# AGENT EXECUTION:
...
#
# ```python
# def _execute_agent_search(
#     self,
#     query: str,
#     entities: List[str],
#     formatted_segments: str
# ) -> Dict[str, Any]:
#     """Call agent LLM to search continuum segments."""
#
#     user_prompt = f"""Search Query: {query}
#
# Entities to look for: {', '.join(entities) if entities else 'N/A'}
#
# Continuum Segments:
# {formatted_segments}
#
# Find information matching the search query. Return JSON with your findings."""
#
#     try:
#         response = self._get_agent_llm_client().generate_response(
#             messages=[
...
#         raw_text = self._get_agent_llm_client().extract_text_content(response)
#         return self._parse_agent_response(raw_text)
#
#     except Exception as e:
#         self.logger.error(f"Agent search failed: {e}")
#         return {"found": False, "confidence": 0.0, "findings": []}
# ```
#
...
#
# ```python
# def _parse_agent_response(self, raw_text: str) -> Dict[str, Any]:
...
#
#     if "found" not in data or "findings" not in data:
...
#         if msg_id and self._validate_message_exists(msg_id):
#             validated_findings.append(finding)
#         else:
#             self.logger.warning(f"Agent cited non-existent message: {msg_id}")
#
...
#
# ```python
# def _validate_message_exists(self, message_id: str) -> bool:
#     """Check if message ID exists in database."""
#     try:
#         db = self._conversation_repo._get_client(self.user_id)
#         rows = db.execute_query(
#             "SELECT 1 FROM messages WHERE id = %s LIMIT 1",
#             (message_id,)
#         )
#         return len(rows) > 0
#     except Exception:
...
#     entities: List[str]
# ) -> Dict[str, Any]:
#     """Format agent findings as search results."""
#
#     if not agent_response.get("found"):
#         return {
#             "status": "not_found",
#             "confidence": 0.0,
#             "query": query,
...
#             "results": [],
#             "meta": {
#                 "search_tier": "agentic_deep_search",
#                 "agent_reasoning": agent_response.get("reasoning")
#             }
#         }
#
#     # Fetch full message data for each finding
#     results = []
#     for finding in agent_response.get("findings", []):
#         msg_id = finding["message_id"]
#
#         # Load message from database
#         message = self._load_message_by_id(msg_id)
#         if message:
#             results.append({
#                 "message_id": str(message["id"])[:8],
#                 "full_uuid": str(message["id"]),
#                 "continuum_id": str(message["continuum_id"]),
#                 "role": message["role"],
#                 "preview": self._truncate_smart(message["content"], 500),
#                 "is_truncated": len(message["content"]) > 500,
#                 "full_length": len(message["content"]),
#                 "timestamp": format_utc_iso(message["created_at"]),
#                 "match_type": finding.get("match_type"),
#                 "match_explanation": finding.get("explanation"),
#                 "match_score": agent_response.get("confidence", 0.5)
#             })
#
#     return {
#         "status": "high_confidence" if agent_response.get("confidence", 0) > 0.7 else "medium_confidence",
...
#             "search_tier": "agentic_deep_search",
#             "agent_reasoning": agent_response.get("reasoning"),
#             "segments_searched": agent_response.get("segments_searched", 0)
#         }
#     }
# ```
#
...
# =============
#
# Add to ConversationSearchToolConfig:
# ```python
# deep_search_enabled: bool = Field(
#     default=False,  # Opt-in initially
#     description="Enable agentic deep search for low-confidence queries"
# )
# deep_search_threshold: float = Field(
#     default=0.40,
#     description="Confidence threshold below which deep search triggers"
# )
# deep_search_max_segments: int = Field(
#     default=5,
#     description="Maximum continuum days to load for deep search"
# )
# deep_search_max_messages: int = Field(
#     default=80,
#     description="Maximum total messages to send to agent"
# )
# ```
#
...
# ==========================
#
# Modify _search_messages():
# ```python
# def _search_messages(self, query: str, entities: Optional[List[str]] = None,
#                      max_results: Optional[int] = None, page: int = 1,
#                      enable_deep_search: Optional[bool] = None) -> Dict[str, Any]:
#
#     # ... existing BM25 search code ...
#
#     confidence = self._calculate_confidence(raw_results, query, entities)
#
#     # Check if deep search should trigger
#     deep_search_enabled = (
#         enable_deep_search
#         if enable_deep_search is not None
#         else self._config.deep_search_enabled
#     )
#
#     if (confidence < self._config.deep_search_threshold
#         and deep_search_enabled):
#
#         self.logger.info(
#             f"Low confidence ({confidence}), triggering deep search"
#         )
#
#         try:
...
#             # Fall back to BM25 results with error note
#
#     # ... rest of existing code ...
# ```
#
# Add to anthropic_schema:
# ```python
# "enable_deep_search": {
#     "type": "boolean",
#     "description": (
#         "Enable agentic deep search if BM25 returns low confidence. "
#         "Agent will read continuum segments and find paraphrased/synthesized info. "
#         "Adds 1-2s latency but handles ambiguous queries better."
#     )
# }
# ```
#
...
# ```python
# def _run_deep_search(self, query: str, entities: List[str],
#                      bm25_results: List[Dict[str, Any]]) -> Dict[str, Any]:
#
#     try:
#         # Select segments
#         dates = self._select_conversation_segments(query, bm25_results)
#
#         if not dates:
#             raise ValueError("No continuum segments to search")
#
#         # Load segments
#         segments = self._load_conversation_segments(dates)
#
#         if not segments or sum(len(msgs) for msgs in segments.values()) == 0:
#             raise ValueError("No messages found in selected segments")
#
#         # Format for agent
#         formatted = self._format_segments_for_agent(segments)
#
#         # Execute agent search
#         agent_response = self._execute_agent_search(query, entities, formatted)
#
#         # Format results
#         return self._format_agent_results(agent_response, query, entities)
#
#     except Exception as e:
#         self.logger.error(f"Deep search failed: {e}", exc_info=True)
#
#         # Fallback: return BM25 results with error note
#         return {
#             "status": "low_confidence",
#             "confidence": self._calculate_confidence(bm25_results, query, entities),
#             "query": query,
#             "entities": entities,
#             "results": [self._format_message_preview(m) for m in bm25_results],
#             "result_count": len(bm25_results),
#             "deep_search_attempted": True,
#             "deep_search_error": str(e),
#             "meta": {
#                 "search_tier": "bm25_fallback_after_agent_failure"
#             }
#         }
# ```
#
...
# ```python
# # At start of deep search
# self.logger.info(
#     "Starting agentic deep search",
#     extra={
#         "query": query,
#         "entities": entities,
#         "bm25_confidence": self._calculate_confidence(bm25_results, query, entities),
...
# self.logger.info(
#     f"Selected {len(dates)} continuum days for deep search",
#     extra={"dates": dates, "total_messages": sum(len(m) for m in segments.values())}
# )
#
...
#     extra={
#         "found": agent_response.get("found"),
#         "confidence": agent_response.get("confidence"),
#         "findings_count": len(agent_response.get("findings", [])),
#         "execution_time_ms": elapsed_ms
#     }
# )
# ```
#
# Return debug metadata in response:
# ```python
# "meta": {
#     "search_tier": "agentic_deep_search",
#     "segments_searched": len(dates),
#     "total_messages_read": total_message_count,
#     "agent_model": "google/gemini-2.5-flash-lite",
#     "execution_time_ms": elapsed_ms,
#     "bm25_hints_used": len(bm25_results) > 0,
#     "agent_reasoning": agent_response.get("reasoning")
# }
# ```
#
...
# ================
#
# 1. Unit tests for segment selection logic
# 2. Integration test with mock agent responses
# 3. End-to-end test with real "Mark's XFS system" query
# 4. Measure: precision (found correct info), recall (found all info), latency
#
# IMPLEMENTATION CHECKLIST:
# ========================
#
# [ ] Add deep search config fields to ConversationSearchToolConfig
# [ ] Implement _select_conversation_segments()
# [ ] Implement _load_conversation_segments()
# [ ] Implement _format_segments_for_agent()
# [ ] Implement _get_agent_llm_client()
# [ ] Write AGENT_SYSTEM_PROMPT
# [ ] Implement _execute_agent_search()
# [ ] Implement _parse_agent_response()
# [ ] Implement _validate_message_exists()
# [ ] Implement _load_message_by_id()
# [ ] Implement _format_agent_results()
# [ ] Integrate _run_deep_search() into _search_messages()
# [ ] Update anthropic_schema with enable_deep_search parameter
# [ ] Add comprehensive error handling
# [ ] Add logging and observability
# [ ] Test with "Mark's XFS system" query
# [ ] Measure performance and cost
#
# ESTIMATED IMPLEMENTATION TIME: 3-4 hours for robust version
