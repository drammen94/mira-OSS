...
- AllMiniLM-L6-v2 (384-dim local embeddings, real-time)
- BGE-M3 (deep embeddings, batch operations)
...
│   ├── hybrid_embeddings_provider.py # AllMiniLM + BGE
...
5. Build weighted context: touchstone + message + message
6. Generate 384-dim embedding (once per turn, shared across services)
7. Surface relevant memories via vector similarity
...
- Summaries embedded (384-dim) for semantic segment search
...
7. Generate embeddings (384-dim AllMiniLM)
...
- contacts_tool, punchclock_tool, continuumsearch_tool
...
**Available Tools**:
- `webaccess_tool`: Playwright-based web scraping (JS rendering)
...
- `continuumsearch_tool`: Semantic search across conversation history
...
**Continuum & Messages**:
- `continuums` - One per user, stores metadata (touchstone, linked_days, etc.)
...
**Long-Term Memory**:
- `memories` - Core memory storage with 384-dim embeddings
...
**Vector Indexes**:
```sql
-- Memory similarity search (AllMiniLM 384-dim)
CREATE INDEX idx_memories_embedding ON memories
    USING ivfflat(embedding vector_cosine_ops);

-- Segment summary search
...
│    - Generate analysis (touchstone) via fast model          │
│    - Build weighted context                                 │
│    - Generate 384-dim embedding (ONCE)                      │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
...
│    - Update continuum metadata (touchstone, embedding)      │
│    - Commit transaction                                     │
│    - Update Valkey cache                                    │
└────────────────────┬────────────────────────────────────────┘
                     │
...
- AllMiniLM for real-time (fast)
- BGE-M3 for batch operations (accurate)
