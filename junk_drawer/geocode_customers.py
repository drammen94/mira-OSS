...
class CustomerGeocoder:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.error("Google Maps API key not found. Set the GOOGLE_MAPS_API_KEY environment variable.")
    ...
    def _load_customer_directory(self) -> Dict[str, Any]:
        ...
        if not self.customer_directory_path.exists():
            logger.error(f"Customer directory cache not found at {self.customer_directory_path}")
            raise ValueError(f"Customer directory cache not found at {self.customer_directory_path}")
        ...
        try:
            with open(self.customer_directory_path, "r") as f:
                ...
                if not isinstance(directory, dict):
                    logger.error(f"Invalid cache format in {self.customer_directory_path}")
                    raise ValueError("Invalid cache format")

                # Ensure we have the expected structure
                if "customers" not in directory:
                    logger.error(f"Invalid cache structure in {self.customer_directory_path}, missing 'customers' key")
                    raise ValueError("Invalid cache structure, missing 'customers' key")

                return directory

        except Exception as e:
            logger.error(f"Error loading customer directory from {self.customer_directory_path}: {e}")
            raise ValueError(f"Error loading customer directory: {e}")
    ...
    def _save_customer_directory(self):
        ...
        try:
            ...
            logger.error(f"Error saving customer directory to {self.customer_directory_path}: {e}")
            raise ValueError(f"Error saving customer directory: {e}")
    ...
    def geocode_address(self, address: Dict[str, str]) -> Optional[Dict[str, float]]:
        ...
        if not address_str:
            logger.error("Empty address string after formatting")
            return None
    ...
    def geocode_customer(self, customer_id: str) -> bool:
        ...
        try:
            ...
            if not address:
                logger.error(f"Customer {customer_id} has no address")
                return False
